 ...
 * Abstract:
 *     A generalized version of printf() that supports arbitrary output
 *     methods and is reentrant, but does not support floating-point editing. 
 * Source:
 *     http://efgh.com/software/gprintf.htm
 *     (as of Wed, 21 Dec 2005 00:32:10 -0600)
 */

/* Modification history:
 *
 * Sun, 12 Feb 2006 04:17:02 -0600
 *   Dwayne Litzenberger <dlitz@dlitz.net>
 *  - Fix a few gcc warnings by adding a few "const" qualifiers
 *  - Add #include "gprintf.h"
    ...
    if (control_char == '%')
    {
      ...
      else
      {
        ...
        else if (base == -2)  /* conversion type s */
        {
          ...
          string = * (char * const *) argument_pointer;
        ...
        else  /* conversion type d, b, o or x */
        {
          ...
          if (long_argument) 
          {
            x = * (const unsigned long *) argument_pointer;
